...
def get_available_engine(
    ...
    if is_multiple_gpus:
        if ddp:
            return DistributedDataParallelEngine(mixed_precision='fp16' if fp16 else 'no')
        else:
            return DataParallelEngine(mixed_precision='fp16' if fp16 else 'no')
    else:
        return GPUEngine(mixed_precision='fp16' if fp16 else 'no')
