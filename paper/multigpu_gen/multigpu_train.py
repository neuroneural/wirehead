...
from catalyst.dl import DataParallelEngine
...
from utils.meshnet import enMesh_checkpoint, enMesh
from utils.clientcreator import ClientCreator
from utils.processing import merge_homologs
...
MODEL_CONFIG = "./conf/modelAE.json"
# Read the YAML file
with open(WIREHEAD_CONFIG, 'r') as file:
    config = yaml.safe_load(file)

# Fetch the desired values
MONGOHOST = config['MONGOHOST']
DBNAME = config['DBNAME']
COLLECTION = config['READ_COLLECTION']
...
# Load params
model_path = ""
wandb_project = sys.argv[1] if len(sys.argv) > 1 else "wirehead_multigpu_dev"
wandb_experiment = sys.argv[2] if len(sys.argv) > 2 else datetime.now().strftime("%Y-%m-%d_%H-%M")
logdir = f"./log/{wandb_experiment}"

# Set up logging
os.makedirs(logdir, exist_ok=True)
sys.stdout = Logger(os.path.join(logdir, "output.log"))

# Start GPU monitoring
stop_event = threading.Event()
gpu_csv_path = os.path.join(logdir, "gpu_metrics.csv")
stop_event = threading.Event()
gpu_monitor_thread = threading.Thread(
    target=gpu_monitor,
    args=(gpu_csv_path, 0.1, stop_event)
)
...
class CustomRunner(dl.Runner):
    ...
    def get_loaders(self):
        ...
        tdataset = MongoheadDataset(config_path = WIREHEAD_CONFIG)
    ...
    def handle_batch(self, batch):
        ...
        sample, label = batch
        if QUANTIZE:
            sample = sample.to(self.engine.device).to(DTYPE)  # Add channel dimension
...
if __name__ == "__main__":
    ...
    start_experiment = 0
    for experiment in range(len(cubesizes))[:1]:
    ...
    # Copy the training script to the log directory
    train_script_name = "train.py"
    shutil.copy(train_script_name, f"{logdir}/{train_script_name}")

    # Stop GPU monitoring
    stop_event.set()
    gpu_monitor_thread.join()

    print(f"Training completed. Logs and outputs saved in: {logdir}")
