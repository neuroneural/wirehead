...
model_label = "_onecycle_largeLR_0weight_high"
...
class CustomRunner(dl.Runner):
    ...
    def get_loggers(self):
        return {
            ...
            #                        name="subcube "+str(subvolume_shape[0])+'_'+COLLECTION+model_label,
            #                        log_batch_metrics=True)
        }
    ...
    def get_criterion(self):
        class_weight = torch.FloatTensor([1.1] + [1.0] * (self.n_classes - 1)).to(self.engine.device)
    ...
    def get_scheduler(self, optimizer):
        ...
        #scheduler = StepLR(optimizer, step_size=len(self.loaders["train"])*10, gamma=0.3)
...
if __name__ == "__main__":
    ...
    onecycle_lr = rmsprop_lr = 0.8/256/6 #0.00001
    n_classes = [104, 3, 50][0]
    n_epochs = 20
    optimize_inline = False
    model_path = ""
    logdir = f"./logs/tmp/enmesh_{model_channels}channels_ELU_largeLR/"
